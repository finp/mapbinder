...
.Overview
To get started with {ProductName} ({ProductShortName}) quickly, the first step is to understand the project life cycle. This involves creating a project, creating a client and a Cloud App from templates, deploying the Cloud App to the MBaaS, building the Client App, and deploying it to a mobile device.

This guide uses the Studio -- the web interface of {ProductShortName} -- for all operations. However, you can also use the link:../dev_tools/local.html[command-line interface] to access most functions of {ProductShortName}.
...
After creating a project, you can see the *Apps, Cloud Apps & Services* page. This shows the Client Apps, Cloud Apps, and MBaaS services associated with a project.
...
The newly created _Hello World Project_ contains one Cordova Client App and one Cloud App with a single HTTP endpoint.
You can add more Client Apps, Cloud apps, and MBaaS services to the project by clicking the `+` symbol in each box.
...
. In the *Apps, Cloud Apps & Services* screen, open the __Cloud App__.
. In the *App Details* screen, see the value of the *Cloud App Status* field.
+
...
To better understand how the Cloud App works, make a small modification to its code. Add a `timestamp` field with the value of the current UNIX time stamp to the server response. In the next section, you will modify the Client App to display the time stamp.

. Navigate to the *Projects* area using the navigation bar at the top.
. Open the _Hello World_ project.
...
----
+
Add the new `<div>` to get the following:
+
[source,markup]
----
<div id="cloudResponse" class="cloudResponse"></div>
...
----
. Save the changes using __File > Save__, or using the _Ctrl + S_ keyboard shortcut.
